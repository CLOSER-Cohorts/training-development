---
title: Unit 1.2 Why create metadata?
collaborators: Jon Johnson, Hayley Mills
date: February 2026
---

# Unit 1.2 Why create metadata?
...
---
Many of the reasons above have been demonstrated in the training course 'Introduction to Metadata', this course will look in more detail how creating metadata could; 
...
From the skills we have aquired through conducting research, analysing data, reading research apapers and through training, we intutively know what to look for when we read the survey to understand the variables, including the meaning behind the words, context and formatting clues. However, if we want to make these decisions more systemactically, look for other oppotunies for different research questions, ensure quality and validity, allow others and our future selves to use the data, compare with other or past studies or prepare for computer processing/machine actionability, it is valuable to record this information and to do so in a strucutred, item level way.  

So how do we capture this information, and what terms might we use? Some of these have been covered in the 'Introduction to Metadata' training course, and will be recapped here, some may be new to you.
...
So far we have pulled out information from the survey documentation about the question, variable and the type of data. As discussed above, there is additional information we know and use to make decisions about the data, which we can record in the metadata. For example, what the variable is measuring and what is the measurement being applied to, this can be seen in the table below in the concept and the unit type. 
...
Similar to the address example given in the 'Introduction to metadata' course (NB this is to be added), we have split each element into a seperate category, we can do this intutively, but documenting this additional metadata in this way can help our understanding even further, and is particulary valuable for more complex datasets or data collections. 
