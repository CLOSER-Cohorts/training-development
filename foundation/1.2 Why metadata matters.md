...
From the skills we have aquired through conducting research, analysing data, reading research papers, and through training, we intutively know what to look for when we read the survey to understand the variables, including the meaning behind the words, context and formatting clues. However, if we want to make these decisions more systemactically, look for other opportunities for different research questions, ensure quality and validity, allow others and our future selves to use the data, compare with other or past studies or prepare for computer processing/machine actionability, it is valuable to record this information and to do so in a strucutred, item level way.  

>[NOTE]
>SW -  I would re-word the second sentence in the above paragrgaph to make it shorter, something like 'It is valuable for us to record the information in a structured, item level way so that we can: and then bullet point from 'make decisions more systematically.../look for other opportunities for different research questions/ ensure quality...etc'
