...
>[!NOTE]
>NOTE for overall course 
> BO - Maybe we could actually use the FAIRwDDI template? I can see the draft you've created is quite similar, but ours follows DDI rather than Dublin Core, so it might be too focused of an example?
> Should we structure Foundation unit around FAIRwDDI template? Currently that template captures less metadata elements so it would slim down the content of Foundaiton
