...
**Valid range**<br>
Valid range outlines the allowed values for this metadata element. For example, for height, you may want to set permitted values to be between 55 - 220 (depending on the population you are collecting data from). For text, you may want to specify how many characters are permitted. For example, first name might be between 1-50 characters. For variables using codelists, you could include a reference to the codelist you are using so people can reference the permissable codes and their meanings (we will look more indepth into codelist metadata in unit 2.4)
...
For the example dataset, we could specify the group of individuals to be <mark>adults aged 18-30</mark> and who are located <mark>in the U.S. as of 2019</mark>, therefore contextualising the broad unit type of individuals.
...
Unlike population, Universe has no specification of time and geography, contextualising a Unit Type. While the Population refers to the group actually studied, the Universe is the larger group that the study's conclusions might apply to, based on the sample. 
...
They may also be defined by what it excludes...<br>
...
What do you think the Unit Type, Population and Universe would be?

Unit type
Households

Population 
Probability sample of approx. 24,000 households in Great Britain in 2009-10

Universe 
questions asked to those in work

What's the benefit of describing Unit Type, Population and Universe 
By capturing Unit Type, Population and Universe metadata, we can compare between different Populations and time by the characteristics defined by the Universe

Those in work in USOC with those in work in the Census<br>


For the example dataset, we would put the following ...
