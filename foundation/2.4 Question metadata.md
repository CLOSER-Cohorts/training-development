...
They way that this metadata is collected will depend on how a questionnaire is administered. For example, if you are using online software to develop and deliver a questionnaire (CAPI questionnaire), you may be able to pull the metadata directly from the online software, saving you time in your metadata creation. However, if a questionnaire is administered as a hard copy, you would need to interpret the paper form that has been developed and create metadata from scratch.
...
**Question text**<br>
The question as written on the original questionnaire. Note, it's important when capturing question text metadata that you don't correct any mistakes in the original research materials, such as a spelling or grammatical errors. It's important to describe the data and the data collection instruments as fully as possible.
...
- **Codelist**
  - When respondents choose from a predefined set of answers, this is called a codelist. You may want to give further information about a codelist to say whether respondents can only select a single option (code) or multiple options.
...
>[!NOTE]
> BO - Something funky is going on with the order of the units :) will need to decide where 2.4 is going and correctly number/position accordingly 
