...
First, let's look at discovery. When do we need find research projects and data?

>[!NOTE]
> BO - suggest changing the the above to "When do we need find existing research projects and their data?", same for the below.
...
>[!NOTE]
> BO - I don't think "data collection process" is needed here "Help inform the research design and data collection process of a new research project" as it's part of the research design stage? Also suggest changing "collect secondary data" to "Identify secondary data".

So, how do we go about finding data? 

Imagine you're a social science researcher looking at population and climate. You want to conduct research into approaches and case studies of improving water quality in cities in the UK. You're considering working from secondary data and you want to find open data from the years 2000-2025 that relate to this subject.

>[!NOTE]
> BO - I don't think "...into approaches and case studies of..." makes sense here, suggest changing to "...conduct research on improving water quality in cities in the UK".
...
>[!NOTE]
> BO - could we link the above to FAIR? It could even just be a sentence to say that this might be ringing some bells from unit 1.3 FAIR.
...
>[!NOTE]
> BO - I think something's missing from the above as it doesn't make sense as is - is the question (when you're saying "Thinking back to the question") this "how do we use a data cataglogue to find relevant research projects relating to our brief."? If so, we just need to maybe put it in quotes and add a question mark at the end. If not, and we're now asking "how do we use a data cataglogue to find relevant research projects relating to our brief.", we need to make that clear. Also, do we need "for example"? That threw me off a bit too. Hope that makes sense, if not, let me know! 

First, we would find a relevant data catalogue. As the brief states we are a social science researcher, we chose the data catalogue UK Data Service. This catalogue stores metadata for economic, population, and social research data in the UK.
...
Outside of research, we use metadata as a discovery tool all the time. For example, on shopping sites, the filters we use to find the product we're after are metadata that have been used to described the product. The role of metadata in helping us sort through large amounts of information and resources, whether that's in research or in our everyday life, 

>[!NOTE]
> BO - I don't think the last sentence is complete, and I don't know if it's needed? It makes sense and is complete without it. I think we should move the real-world example to the start of this section if we decide to keep it - I think this unit makes sense and flows nicely without it. Save it for later either way though, as we could even move it to an earlier unit as I think real world examples are helpful for contextualising what metadata is and how it can be used.

>[!NOTE]
>SW I think we should keep this example in, as it’s a real world one illustrating the filtering function. If it were to be moved, you could place it as an example (with the associated paragraph) at the end of the Using metadata to discover and understand data section, so the person has an immediate contrast between research metadata filtering and ‘real world’ shopping filtering(!) for want of a better phrase…
...
Data catalogues most commonly provide high level metadata. As we explored in [unt 2.1](<2.1 Introduction to metadata.md>), having both metadata about the research project (high level metadata) and the data and data collection processes (low level metadata) is useful for being able to fully understand a research project.

So, how do we find metadata about data?

>[!NOTE]
> BO - Suggest changing the above to "So, how do we find metadata at the data level?" or something like that. I'd also argue that metadata about the data collection process is high-level if you're referring to the research methods used, but if you're referring to the questionnaires then yes that's low level too. We might need to be more explicit or just remove "data collection processes" to avoid confusion.
...
Another way we can access low level metadata is through additional documentation stored alongside datasets. These types of documentation are called codebooks or data dictionaries. These can be stored as downloadable resources on data repository and data catalogue sites on the research project profile page. For example ...
