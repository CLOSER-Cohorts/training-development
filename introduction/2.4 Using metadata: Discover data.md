...
First, let's look at discovery. When do we need find existing research projects and their data?
...
Outside of research, we use metadata as a discovery tool all the time. For example, on shopping sites, the filters we use to find the product we're after are based on a product's metadata. In this way, metadata helps us sort through large amounts of information and resources.
...
>[!NOTE]
> **KR DECIDE WHERE TO MOVE EXAMPLE OR REMOVE ALTOGETHER**
...
Data catalogues most commonly provide high level metadata about the project. As we explored in [unt 2.1](<2.1 Introduction to metadata.md>), having  further metadata about the variables within a project is useful for being able to fully understand what type of data a project contains.
...
Both codebooks and data dictionaries bring together variable and question metadata in order to give a robust overview of what data is included in a research project or dataset. If a project didn't create this metadata, it wouldn't be possible to create these types of documentation. Often, codebooks and data dictionaries can be automatically generated from metadata that is already created and managed as part of the research process.
