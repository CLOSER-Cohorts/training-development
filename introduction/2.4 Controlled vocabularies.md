...
- **Interoperability** of metadata with other studies' so it can be compared across resources and despotied on centralised platforms such as data catalogues where it can be searched and filtered
...
You can create your own controlled vocabulary by setting a list a set of accepted terms or values for a metadata element. For the colour metadata in the previous example, we could use the controlled vocabulary of...
...
This would remove the issue of including confusing categories ('pale blue' and 'light blue') and prevent the formatting error of 'Green' and 'green'. However, while we have restricted what information someone can select, we don't how objects are classified into different categories or what this categories mean.

As such, it's important to provide definitions so we know what terms mean. 
...
There are also publicly available controlled vocabularies that provide a restricted list of terms that are already pre-defined terms so you don't have to write out the defintions yourself. These lists relate to a specific discipline or area of knowledge and they are created and updated by a relevant research community and are a type of community based standard. 
