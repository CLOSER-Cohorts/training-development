...
As we touched on in the end of the last unit, metadata can be leveraged as a search tool, allowing people to search resources based on keywords. Metadata terms can also be used as filters to explore a large set of objects or data. Through being machine-actionable in this way, metadata provides more 
...
Inconsistent or incorrect terms also makes metadata less interoperable. 'Dirty' metadata can be a barrier to depositing your (meta)data in a data repository or catalogue as it cannot integrate with the centralised system.
