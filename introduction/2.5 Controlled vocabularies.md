...
As we explored in the last unit, metadata can be used as a search tool to discover and understand research projects. However, this is only possible because metadata is machine actionable. If it wasn't machine actionable, data repositories and data catalogues could not offer the search and filter functions.

In order to be machine actionable, metadata needs to be structured and stored in appropriate machine-readable formats, for example: CSV, XML, JSON and MS Excel rather than free- text formats such as PDF or MS Word. 
...
To avoid this from happening, people who create metadata should follow best practices to ensure their metadata is effective. So, what is best practice and how do we know what best practice to follow?
