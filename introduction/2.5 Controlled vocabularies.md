...
As we explored in the [last unit](<2.4 Using metadata: Discover data.md>), metadata can be leveraged as a search and filter tool on data catalogues. This enables people to find data based on keywords, topics or dates. Through being machine actionable in this way, metadata is a powerful form of documentation that provides more functionality than unstructured documentation.
...
In [unit 2.1](<2.1 Introduction to metadata.md>) we touched on machine readable formats that metadata should be stored in. This includes CSV, XML, JSON and MS Excel. By being in these formats, machines can read metadata and action it. Metadata cannot be stored in free-text formats such as PDF or MS Word as computers will not be able to read and interpret this information. 
...
'Dirty' metadata fails to effectively implement both the FAIR data principles and the main purpose of metadata to increase discovery, understanding and (re)usability of data.
...
They provide consistency and structure to metadata, reducing the risk of duplicate terms where different terms are used for the same concept or one term is used for multiple concepts. A controlled vocabulary relates to a specific discipline or area of knowledge.
...
The controlled vocabulary, [ISO 3166-1 Alpha-2 Country Codes](https://www.iso.org/obp/ui/#search), offers a list of two-letter codes for every country which can be used across languages, allowing us to standardise how we refer to countries across different research projects 
- US – United States
- DE – Germany

>[!NOTE]
> Currently using NBI Sweden definition of controlled vocabulary. I think it gives a bit more depth than the CODATA definition which is this 'List of standardised terminology, words, or phrases, used for indexing or content analysis and information retrieval, usually in a defined information domain.'<br>
...
-Languages
 - For metadata that desrcibes that language the data is stored in
- File format
  - For metadata that desrcibes what format the data is stored in
- Access and copyright
  - For metadata that desrcibes a research project's access and copyright licenses

**Discipline specific** <br>
You may use controlled vocabularies for discipline specific metadata such as...
- Metadata describing data collection method or instruments e.g. Questionnaries, Surveys, Focus Groups 
- Metadata using specialised terms and/or categories e.g. disease names in life sciences
- Metadata describining keywords or topics specific to your discipline 
- Metadata referencing standardised scales or assessments used in a particular field
...
You can also find controlled vocabularies yourself, using websites such as [Bartoc](https://bartoc.org/) which compiles many controlled vocabularies allowing you to search and filter different across disciplines and concepts to identify the most appropriate one for your data. Your discpline may also have a discpline specific search site, for example [The NERC Vocabulary Server (NVS)](https://vocab.nerc.ac.uk/search_nvs/) which allows you to search all relevant vocabularies for marine and related earth science domains.
...
## Practice using a controlled vocabularly 

>[!NOTE]
> WANT TO ADD A PRACTISE EXERCISE PROVIDING PARTICIPANTS WITH A SMALL TABLE OF HIGH LEVEL METADATA THAT CAN APPLY A CONTROLLED VOCABULARY TO (LIKE THE BOOK EXAMPLE BUT METADATA IS RELATED TO A RESEARCH PROJECT). WANT TO DISCUSS WHAT BEST METADATA TO USE TO EXPLORE THIS.
...
As we covered earlier in this unit, controlled vocabularies offer **one term** to describe one concept. In comparison, theasuri offer different terms that are commonly used for the same concept. They may specify what the preferred term to use. 
...
Classification groups similar items or terms into categories, defining the parameters of each category. These categories can be flat or hierarchial. Classifications are most often discipline speicfic.
...
Where taxonomies are focused on defining hierarchial relationships, ontologies have a wider reach and describe relationships between concepts and objects beyond hierarchial terms. It may specify if an object is 'part of' something else or 'equaivalent to' for example. In this way, it is a more sophisticated and detailed description of relationships between items.
...
> :large_orange_diamond: KR I think it should be included here as on Bartoc's website / other CVs repository websites, Theasuri, taxonomies and ontologies are stored. So if we include the exercise where participants go and look for a relevant CV, I think we should cover these terms. Though can be high-level. However, happy to remove if we think it's too complex for this course. 
