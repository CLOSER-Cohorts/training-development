...
- Accurate descriptions so data is easily understood
...
Controlled vocabularies provide consistency and structure to metadata, reducing the risk of duplicate terms where different terms are used for the same concept, or one term is used for multiple concepts. A controlled vocabulary relates to a specific discipline or area of knowledge.

>[!NOTE]
> BO - "where different terms are used for the same concept, or one term is used for multiple concepts" could we add and an example or reference back to the trainers/sneakers one? - Just seen the example below - can we move it so it comes directly after that sentence, so we'd have "Controlled vocabularies provide consistency...For example, a country..."?
> BO - "A controlled vocabulary relates to a specific discipline or area of knowledge." this feels a bit random. Suggest integrating with the first para e.g. "Controlled vocabularies are a type of community based standard which relate to a specific discipline or area of knowledge."
...
> BO - If we're using CODATA throughout, you could use CODATA and then follow up with a sentence referncing the additional info NBI Sweden provides, one as I think this bit "In the controlled vocabulary you **only use one term to describe one particular phenomenon**, excluding all other synonyms." is helpful.
> JJ lets use CODATA
...
> JJ LOC is very well established and fine to use 
...
- Metadata describing keywords or topics specific to your discipline 
...
>[!NOTE]
>SW The second point 'Languages' is not displaying properly - the explanation for it has been assigned a black bullet point instead
---
...
>[!NOTE]
>SW re sentence above - should it be unit 2.6?

You can also find controlled vocabularies yourself, using websites such as [Bartoc](https://bartoc.org/) which compiles many controlled vocabularies allowing you to search and filter across disciplines and concepts to identify the most appropriate one for your data. Your discpline may also have a discpline specific search site, for example [The NERC Vocabulary Server (NVS)](https://vocab.nerc.ac.uk/search_nvs/), which allows you to search all relevant vocabularies for marine and related earth science domains.
...
While controlled lists are a flat list that do not specify relationships between terms, taxonomies and ontologies describe the connections between terms. This includes the hierarchy of terms, for example a term that has a group of sub-terms related to it (parent/child term). 

>[!NOTE]
>SW can you explain a bit more epxlicitly the 'parent/child' term? As in what makes this a sub-term. Just a sentence
...
> BO - I think it looks good - I like that it's in the "going further" section and there's just enough detail to be informative but not overwhelming. :)
