...
Why is high level metadata important for us in understanding a research project and determining whether if its relevant for our research activities?
...
Now we know the high level information about how the dataset, do we have enough information to know if the project's dataset would be relevant and/or useful for our secondary research?
What other information do we need to effectively assess if a project's datasets are useful for us?
...
Having information about what the dataset contains, e.g. information about the different types of data and what they capture can further let us know if we want access a dataset. For example, the two datasets below can both be described as XXX.

> :large_blue_diamond: KR Add comparison of two tables that contain data that both relate to the same high level metadata but use different variable / unit of measurement. Link it back to example of secondary research.
...
Thinking about to example dataset, by answering the blue bubbles, metadata can describe the data itself. This can be referred to as data-level metadata (see unit 2.2 for more information on the different levels of metadata)
