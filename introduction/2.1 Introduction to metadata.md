...
But first, let's start with an imagined scenario.
...
> BO - It reads really well to me now - the link between the can and research data is much clearer :)

---
## Understanding a mystery dataset

Imagine you're conducting a research project exploring the behaviours of young people in 1965-1970 in the Northern regions of England. You'll use secondary data and need to identify relevant datasets for your project.

You come across this dataset and need to determine whether it would be useful for your research.
...
> BO - Hmmm, I think we can illustrate the point without including that variable and having to explain that it's from a different dataset, do you agree? If so we can remove it. Just seen below that we link to the dataset in Discovery, so yes if we're linking to the dataset I think we should only include variables in that dataset.
> JJ - only 3 people in the world will notice, but lets remove
...
Thinking back to the example dataset, by answering the blue bubbles, metadata describes the data itself. This can be referred to as low-level metadata.

> [!NOTE]
> BO - The above sounds a bit repetitive, suggest rephrasing to something like "Thinking back to the example dataset, metadata can help us answer the blue bubble questions related to the data itself (i.e. the low level metadata)."
...
As an evolving field, it's importnant to note that terminology around metadata can vary depending on the source you're using; the same term can be used or interpreted by people to mean slightly different things. 
...
> BO - After the restructure I think it flows nicely here, what do you think? We could always use CODATA in previous sections when we're providing definitions, but then go into the details about the different definitions and understandings of metadata here.
...
For people using metadata, metadata can help you...
...
The FAIR Aware Tools consists of 10 questions that will take between 10-30 mins to complete. Using the answers you provide, the tool assess your awareness level, as well as providing tailored tips relevant to your role on how you can further improve your FAIR skills.
