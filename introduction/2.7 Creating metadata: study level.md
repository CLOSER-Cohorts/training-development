...
Dublin Core is used across disciplines to document a range of resources, describing what the resource is, when it was created, who created it and how it can be accessed. The standard is expressed as a simple schema which contains 15 core elements. Depending on the resource you want to describe, some of the metadata elements may not be relevant. In these cases, you can adjust what metadata elements to include accordingly.
...
While you can create Dublin Core metadata in an Excel spreadsheet, Dublin Core also provides an online generator where you can create metadata in a machine-readable file. It also provides quick and easy links to definitions and guidance for each metadata element.
